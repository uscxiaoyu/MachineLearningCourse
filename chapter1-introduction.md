...
$$
显然有$e_{test}+r_{test}=1$。通常将机器学习方法对未知数据的预测能力称为泛化能力。
...
- 过拟合是指学习时选择的模型所包含的参数过多，从而对已知数据预测很好，但对未知数据预测得很差的现象。模型选择旨在避免并提高模型的预测能力。
...
- 下面给出一个简单的泛化误差上界的例子：二分类问题的泛化误差上界。考虑二分类问题，给定训练集$T=\{(x_1,y_1),(x_2,y_2),\mathellipsis,(x_N,y_N)\}$，其中$x_i\in \mathrm{R}$是$y$的观测值，$i=1,2,\mathellipsis,N$，N是样本容量，T是从联合概率分布$P(X,Y)$独立同分布产生的，$X\in \mathbb{R^n},Y\in\{-1,+1\}$。假设空间是函数的有限集合$F=\{f_1,f_2,\mathellipsis,f_d\}$，`d`是函数个数。设$f$是从$F$中选取的函数。损失函数是$0-1$损失。
