...
- `k`近邻法(`k nearest neighbor, kNN`)是一种基本分类和回归方法(`Cover & Hart, 1968`)。

- `k`近邻法的输入为实例的特征向量，对应于特征空间的点，输出为实例的类别，可以取多类。

- 分类时，对新的实例，根据其`k`个最近邻的训练实例的类别，通过**多数表决等方式**进行预测。

- `k`近邻法不具显式的学习过程，`k`值的选择、距离度量以及分类决策规则是`k`近邻法的三要素。

- 难点在于如何高效地定位到输入实例的`k`个最近邻居。
...
- $L_p$距离
    设特征空间$\mathbf{X}$是$n$维实数向量空间$\mathbf{R^n}$，$x_i, x_j\in \mathbf{x}, x_i = (x^{(1)}_i,x^{(2)}_i,...,x^{(n)}_i)^T,x_j=(x_j^{(1)}, x_j^{(2)},... ,x_j^{(n)})^T,x_i,x_j$的$L_p$距离定义为
    $$
...
- `kd`树是一种对`k`维空间中的实例点进行存储以便对其进行快速检索的树型数据结构。`kd`树是二叉树，表示对`k`维空间的一个划分。构造`kd`树相当于不断地用垂直于坐标轴的超平面将`k`维空间切分，构成一系列的`k`维超矩形区域。
...
# 3. `kd`树: 构造算法
**算法3.2（构造平衡`kd`树）**
- 输入：`k`维空间数据集$T=\{(x_1, y_1), (x_2, y_2), ..., (x_N, y_N)\}$，其中$x_i = (x^{(1)}_i,x^{(2)}_i,...,x^{(n)}_i)^T,i=1,2,...,N$
- 输出：`kd`平衡树
- 算法过程：
...
- `k`近邻法是一种基本的分类与回归方法
- `k`近邻法的三要素：距离度量、`k`值的选择和分类决策规则
- `kd`树是一种对`k`维空间中的实例点进行存储以便对其进行快速检索的树型数据结构
  - `kd`树是二叉树，表示对`k`维空间的一个划分
- `kd`树的构造
  - 是一个递归的过程，选择坐标轴对空间划分，选择训练实例点在选定坐标轴上的中位数为切分点
- `kd`树的搜索
  - 是一个递归的过程，从根结点出发，递归地向下访问`kd`树，直到叶结点为止
  - 是一个递归的过程，从叶结点回退，寻找距离`x`最近的`k`个近邻
- `k`近邻法的决策规则是多数表决，对于回归问题，是近邻的平均值
