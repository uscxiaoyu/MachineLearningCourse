...
- 假定输入纬度为d纬向量x，输出值为x，则RBF网络可表示为
    $$
    \varphi(x)=\sum_{i=1}^qw_i\rho(x,c_i)
    $$
    其中q为隐层神经元数量，$c_i$和$w_i$分别是第i个隐层神经元对应的中心和权重，$p(x,c_i)$是径向基函数。
---
# 其它常见神经网络
## 1. RBF网络
- $p(x,c_i)$是某种沿径向对称的标量函数，通常定义为样本x到数据中心$c_i$之间欧氏距离的单调函数。常用的高斯径向基函数
$$
...
# 其它常见神经网络
## 3. SOM网络
- SOM（Self-Organizing Map, 自组织映射）是一种竞争学习型的无监督神经网络，它能将高纬输入数据映射到低纬空间，同时保持输入数据在高纬空间的拓扑结构，即将高纬度空间中相似的点映射到网络输出层中的邻近神经元
- SOM网络中的输出层神经元以矩阵方式排列在二维空间中，每个神经元都拥有一个权向量，网络在接收输入向量后，将会确定输出层获胜神经元，它决定了该输入向量在低维空间的位置
...
## 3. SOM网络
- SOM网络的训练过程：
  - 在接收到一个训练样本后，每个输出层神经元会计算该样本与自身携带的权向量之间的距离，距离最近的神经元称为竞争获胜者，称为最佳匹配单元（best matching unit）
...
- 这样的结构与信息反馈过程，使得网络在t时刻的输出状态不仅与t时刻的输入有关，还与t-1时刻的网络状态有关，从而能处理与时间相关的动态变换
...
# 其它常见神经网络
## 6. Bolzmann机
- 神经网络中有一类模型是为网络状态顶一个“能量”，能量最小化时网络达到理想状态，而网络训练就是在最小化这个能量函数
...
# 其它常见神经网络
## 6. Bolzmann机
- 令向量$S\in\{0,1\}^n$表示n个神经元的状态，$w_{ij}$表示神经元i与j之间的连接权，$\theta_i$表示神经元i的阈值，则状态向量S所对应的Bolzmann机能量定义为
$$
...
- 现实中常采用受限Bolzmann机（Restricted Bolzmann Machine， RBM），RBM仅保留显层与隐层之间的连接，从而将Bolzmann机结构由完全图简化为二部图。
- RBM常用对比散度(Contrastive Divergence, CD)算法（Hinto, 2010）来进行训练
