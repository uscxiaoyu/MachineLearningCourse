...
- 1958年，计算科学家Rosenbalt提出了由两层神经元组成的神经网络，取名为Perceptron

---
# 1. 神经网络的发展过程
- 1969年，人工智能巨擘Minsky在"Perceptron"一书中，证明了感知机不能解决异或问题的弱点，且认为如果将计算层增加到两层，计算量将很大，且没有有效的学习算法，由此认为研究更深层的网络没有价值

- 1986年，Rumelhart和Hinton等人提出反向传播算法，解决了两层神经网络所需要的复杂计算量问题，从而带动了业界使用两层神经网络研究的热潮。十余年前困扰神经网络界的异或问题被轻松解决，神经网络已经可以用于语音识别、图像识别和自动驾驶等多个领域
...
---
# 1. 神经网络的发展过程
- 2012年，Hinton和他的学生在ImageNet竞赛中，用多层的卷积神经网络成功地对包含1,000多个类别的1,000,000多张图片进行了训练，取得了分类错误率15%的好成绩，比第二名高出11个百分点
...
---
# 1. 神经网络的发展过程
- 2018年，Hinton, Lecun, Bengio等三位深度学习科学家因在神经网络方面的基础性发现和发明获得诺贝尔物理学奖
- 2019年，OpenAI发布了一个基于Transformer的神经网络模型GPT-2，可以生成逼真的文本，从而在自然语言处理领域取得了巨大的成功
- 2020年，Google的DeepMind发布蛋白质结构预测模型AlphaFold，可预测蛋白质的三维结构，从而在生物领域取得了巨大的成功
...
- 2025年1月，中国的深度求索公司基于DeepSeek-V3-Base，通过组相对策略优化（GRPO）优化，推出了DeepSeek-R1模型，同时开放模型权重
- 2025年7月，OpenAI发布GPT-5，同年9月发布Sora 2视频生成模型
- 2025年9月，DeepSeek-R1模型登上《Nature》封面

---
# 2. 神经网络的结构
- 一个生物神经细胞的功能比较简单，而人工神经元只是生物神经细胞的理想化和简单实现，功能更加简单。要想模拟人脑的能力，单一的神经元是远远不够的，需要通过很多神经元一起协作来完成复杂的功能

- 神经网络是通过一定连接方式进行协作的神经元系统。到目前为止，研究者已经发明了各种各样的神经网络结构。
- 目前常用的神经网络结构有以下三种：
    - 前馈神经网络
    - 反馈神经网络
    - 图神经网络
...
- 记忆网络也称为反馈网络，网络中的神经元不但可以接收其他神经元的信息，也可以接收自己的历史信息

- 和前馈网络相比，记忆网络中的神经元具有记忆功能，在不同的时刻具有不同的状态。记忆网络中的信息传播可以是单向或双向传递，因此可用一个有向循环图或无向图来表示

- 记忆网络包括循环神经网络、Hopfield 网络、玻尔兹曼机、受限玻尔兹曼机等

- 记忆网络可以看作一个程序，相对函数而言，具有更强的计算和记忆能力
...
- 前馈神经网络（`Feedforward Neural Network, FNN`）是最早被发明的简单人工神经网络
...
- 神经网络有点像C语言，能够表达任何可计算的程序。 然而，想出一个符合规范的程序才是最困难的部分
...
- 解决问题的一个方法是引入非线性变换
  - 例如：对隐藏变量使用按元素运算的非线性函数进行变换，然后再作为下一个全连接层的输入。

- 这个非线性函数被称为**激活函数（activation function）**
...
- 当输入为负数时，ReLU函数的导数为0；当输入为正数时，ReLU函数的导数为1。尽管输入为0时ReLU函数不可导，但是可以取此处导数为1
...
- ELU函数在图像分类、自然语言处理和机器翻译等任务中应用广泛
...
- 交叉熵损失函数：应用于多分类任务
    ...
    def cross_entropy(y, hat_y):  # 交叉熵
        ...
        '''
        crossEnt = -torch.dot(y.reshape(-1), torch.log2(hat_y).reshape(-1)) / y.shape[0]
        return crossEnt
    ```
...
---
# 7. 其它常见神经网络
除多层感知机之外，在漫长的神经网络发展历程中，也涌现出其它形式的神经网络
## (1) `RBF`网络
...
- 若输入向量与获胜神经元代表向量之间的相似度大于识别阈值，则当前输入样本被归为神经元所属类别，同时连接权重将更新，使以后在接收到类似样本时该神经元会计算出更大相似度，从而使获胜神经元有更大获胜可能性
...
- $\theta(i, i_{BMU}, t)$用于确定第$i$个神经元和BMU之间的邻域关系，该函数值随着距离增加而减小，通常采用高斯函数形式
...
## (3) SOM网络: 权重更新
- 权重更新有两个主要的作用：
    - **自组织特性**：通过逐渐调整权重，神经元自动组织其位置以反映输入数据的内在结构
    - **拓扑保持**：通过保持输入空间中邻近样本的相对位置关系，SOM能够在其映射的低维网格中保持输入数据的拓扑结构
...
# 7. 其它常见神经网络
## (6) Bolzmann机
- 假定网络中有$d$个显层神经元和$q$个隐层神经元，令$v$和$h$分别表示显层和隐层的状态向量，则由于同一层内不存在连接，有
$$
\begin{gather*}
...
# 7. 其它常见神经网络
## (6) Bolzmann机
- CD算法对每个训练样本$v$，先根据上式计算出隐藏层神经元状态的概率分布，然后根据这个概率分布采样得到$h$
- 此后，根据上式从$h$产生$v'$，再从$v'$产生$h'$，连接权的更新公式为
$$
...
- 可以直接用它来测试机器学习和深度学习算法性能

---
# 8. `Fashion-MNIST`数据集

- 经典的`MNIST`数据集包含了大量的手写数字。来自机器学习、机器视觉、人工智能、深度学习领域的研究人员把这个数据集作为衡量算法的基准之一

- 在很多的会议，期刊的论文中发现这个数据集的身影。实际上，`MNIST`数据集已经成为算法作者的必测数据集之一

- 有人曾调侃道："如果一个算法在`MNIST`不work, 那么它就根本没法用；而如果它在`MNIST`上work, 它在其他数据集上也可能不work！"

- `Fashion-MNIST`的目的是要成为`MNIST`数据集的一个直接替代品。作为算法作者，不需要修改任何的代码，就可以直接使用这个数据集。`Fashion-MNIST`的图片大小，训练、测试样本数及类别数与经典`MNIST`完全相同
...
```
> 得到`numpy.ndarray`形式的图片数据及其标签。格式遵循`PIL`，即1张图片有三个维度构成，分别为高$H$, 宽$W$, 通道$C$
...
def load_mnist(path, kind='train'):
    ...
    with gzip.open(labels_path, 'rb') as lbpath:
         # 4个字节（32位整数）保存magic number，后4个字节保存标签数量
        labels = np.frombuffer(lbpath.read(), dtype=np.uint8, offset=8)
...
print(f'Train accuracy: {accuracy(train_labels, 
                                multilayer_perceptron(train_features, layers)):.4f}') 
print(f'Test accuracy: {accuracy(test_labels, 
                                multilayer_perceptron(test_features, layers)):.4f}') 
```
